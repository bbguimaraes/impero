...
size_t update_visible(QLayout *l, const QString &filter) {
    if(filter.isEmpty()) {
        for(int i = 0, n = l->count(); i < n; ++i)
            if(auto *w = widget(l, i))
                w->setVisible(true);
        return static_cast<size_t>(l->count());
    }
    size_t ret = {};
    for(int i = 0, n = l->count(); i < n; ++i)
        if(auto *w = static_cast<QLabel*>(widget(l, i))) {
            ...
            ret += static_cast<size_t>(visible);
        }
    return ret;
}
...
template<bool S>
void set_selected(QLayout *l, size_t i) {
    set_selected<S>(static_cast<QLabel*>(widget(l, i)));
}

template<bool forward>
size_t move_selection(QLayout *l, size_t i) {
    set_selected<false>(l, i);
    const auto n = static_cast<size_t>(l->count());
...
void Panel::select_command() const {
    if(!this->n_visible)
        return;
    assert(this->cur < static_cast<size_t>(this->layout()->count()));
